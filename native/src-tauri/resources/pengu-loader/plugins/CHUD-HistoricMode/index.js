...
(function initHistoricMode() {
  ...
  // Shared bridge (provided by CHUD-SkinMonitor)
  let bridge = null;
  ...
  const CSS_RULES = `
    .skin-selection-item-information.loyalty-reward-icon--rewards.chud-historic-flag-active {
  ...
  function injectDialogFrameStyles() {
    const styleId = "chud-historic-mode-dialog-frame-styles";
  ...
  function handleCustomModStateUpdate(data) {
    if (data.active && data.modName) {
      ...
      const currentSkinId = Number((window.__chudSkinState || {}).skinId);
  ...
  function updateHistoricFlag() {
    ...
    if (historicModeActive) {
      ...
      element.classList.add("chud-historic-flag-active");
  ...
  function hideFlagOnElement(element) {
    ...
    element.classList.remove("chud-historic-flag-active");

    // Check if random flag is active - if so, don't remove shared styles
    const hasRandomFlag = element.classList.contains("chud-random-flag-active");
