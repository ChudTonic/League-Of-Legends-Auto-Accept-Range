...
(function initPartyMode() {
  ...
  const BRIDGE_PORT_STORAGE_KEY = "chud_bridge_port";
  const DISCOVERY_START_PORT = 50000;
  const DISCOVERY_END_PORT = 50010;

  const PANEL_ID = "chud-party-panel";
  const BUTTON_ID = "chud-party-button";
  const LOBBY_BUTTON_ID = "chud-party-lobby-button";
  ...
  function injectStyles() {
    const styleId = "chud-party-mode-styles";
        ...
        peersList.innerHTML = allPeers
          .map((peer) => {
            ...
            <div class="peer-item" data-summoner-id="${peer.summoner_id}">
              ...
              <button class="peer-remove" title="Remove" onclick="window.chudPartyRemovePeer(${peer.summoner_id})">
  ...
  window.chudPartyRemovePeer = function (summonerId) {
