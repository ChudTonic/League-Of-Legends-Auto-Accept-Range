...
(function createCustomWheel() {
  ...
  const BUTTON_CLASS = "chud-chroma-button";
  const BUTTON_SELECTOR = `.${BUTTON_CLASS}`;
  const PANEL_CLASS = "chud-chroma-panel";
  const PANEL_ID = "chud-custom-wheel-panel-container";
  const REQUEST_TYPE = "request-skin-mods";
  const EVENT_SKIN_STATE = "chud-skin-monitor-state";
  ...
  function setRightPaneMode(mode) {
    ...
    if (panel._rightTitle) {
      if (mode === "picker") {
        ...
        panel._rightTitle.innerHTML = `<span class="chud-wheel-title-icon">${icon}</span> Choose \u2022 ${escapeHtml(getTabLabel(activeTab))}`;
      } else {
  ...
  // Shared bridge API (provided by CHUD-SkinMonitor)
  let bridge = null;
  ...
  const CSS_RULES = `
    ...
    lol-uikit-flat-button.chud-custom-wheel-button,
    .chud-custom-wheel-button {
    ...
    .chud-custom-wheel-button .count-badge.social-count-badge,
    lol-uikit-flat-button.chud-custom-wheel-button .count-badge.social-count-badge,
    .chud-custom-wheel-button > .count-badge.social-count-badge,
    lol-uikit-flat-button.chud-custom-wheel-button > .count-badge.social-count-badge {
      position: absolute !important;
      /* Positioning: edit these via CSS variables on the element (DevTools-friendly) */
      top: var(--chud-badge-top, -4px) !important;
      right: var(--chud-badge-right, -17px) !important;
      left: var(--chud-badge-left, auto) !important;
      ...
      z-index: 1 !important;
      transform: translate(
        var(--chud-badge-translate-x, 60%),
        var(--chud-badge-translate-y, -60%)
      ) !important;
      margin: 0 !important;
    ...
    .${PANEL_CLASS} .chud-wheel-right-header {
      display: flex;
      align-items: center;
    ...
    .${PANEL_CLASS} .chud-wheel-right-title .chud-wheel-title-icon {
      width: 18px;
      height: 18px;
      flex-shrink: 0;
    }

    .${PANEL_CLASS} .chud-wheel-right-title .chud-wheel-title-icon svg {
      width: 18px;
      height: 18px;
      fill: none;
      stroke: #c8aa6e;
    ...
    .${PANEL_CLASS} .chud-wheel-summary {
      flex: 1;
      min-height: 0;
      display: flex;
      flex-direction: column;
    ...
    .${PANEL_CLASS} .chud-wheel-summary-icon svg {
      width: 18px;
      height: 18px;
      fill: none;
      stroke: currentColor;
    ...
    .${PANEL_CLASS} .chud-wheel-summary-left {
      flex: 1;
      min-width: 0;
      display: flex;
      flex-direction: column;
    ...
    .${PANEL_CLASS} .chud-wheel-picker {
      flex: 1;
      min-height: 0;
      display: none;
    }

    .${PANEL_CLASS} .chud-wheel-picker.active {
      display: flex;
      flex-direction: column;
      min-height: 0;
    }
  ...
  function injectCSS() {
    const styleId = "chud-custom-wheel-css";
  ...
  function createButton() {
    ...
    button.className = "lol-uikit-flat-button idle chud-custom-wheel-button";
    ...
    countBadge.style.setProperty("--chud-badge-top", "-4px");
    countBadge.style.setProperty("--chud-badge-right", "-17px");
    countBadge.style.setProperty("--chud-badge-left", "auto");
    countBadge.style.setProperty("--chud-badge-translate-x", "60%");
    countBadge.style.setProperty("--chud-badge-translate-y", "-60%");
  ...
  function createPanel() {
    ...
    rightHeader.className = "chud-wheel-right-header";

    const rightTitle = document.createElement("div");
    rightTitle.className = "chud-wheel-right-title";
    ...
    summaryView.className = "chud-wheel-summary";
    ...
    SUMMARY_TABS.forEach((tab) => {
      ...
      row.className = "chud-wheel-summary-row";

      // Left cell: value
      const left = document.createElement("div");
      left.className = "chud-wheel-summary-left";

      const label = document.createElement("div");
      label.className = "chud-wheel-summary-label";
      label.style.display = "flex";
      label.style.alignItems = "center";
      label.style.gap = "6px";

      const iconSpan = document.createElement("span");
      iconSpan.className = "chud-wheel-summary-icon";
      ...
      value.className = "chud-wheel-summary-value";
    ...
    pickerView.className = "chud-wheel-picker";
  ...
  function handleModSelect(modId, buttonElement, modData) {
    ...
    if (selectedModId === modId) {
      ...
      const state = window.__chudSkinState || {};
      ...
      if (championId && emitSkinId) {
        ...
        console.log(`[CHUD-CustomWheel] Sending mod selection:`, payload);
        if (bridge) bridge.send(payload);
      } else {
        console.warn(`[CHUD-CustomWheel] Cannot send mod selection - missing championId or skinId:`, { championId, skinId: emitSkinId });
      }
    }
  ...
  // Backend should look in: %LOCALAPPDATA%\Chud\mods\maps
  ...
  // Backend should look in: %LOCALAPPDATA%\Chud\mods\fonts
  ...
  // Backend should look in: %LOCALAPPDATA%\Chud\mods\announcers
  ...
  // Backend should look in: %LOCALAPPDATA%\Chud\mods\<category>
  ...
  function handleModsResponse(event) {
    ...
    const liveSkinId = Number((window.__chudSkinState || {}).skinId) || skinId;
